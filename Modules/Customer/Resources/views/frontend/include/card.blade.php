        ...
        @php
            $price = $take->getProduct->price;
            $sale = $take->getProduct->sale_price;
            $stock = $take->getProduct->stock;
            ...
            <div class="product-card animate-underline hover-effect-opacity bg-body rounded">
                ...
                <div class="position-relative">
                    ...
                    <a class="d-block rounded-top overflow-hidden p-3 p-sm-4"
                       href="{{ route('product_detail', $take->getProduct->slug . '-' . $take->getProduct->product_token) }}">

                        <div class="ratio" style="--cz-aspect-ratio: calc(240 / 258 * 100%)">
                            <img src="/upload/product/{{ $take->getProduct->image }}"
                                 onerror="this.src='/extra/img/photo.png'"
                                 alt="{{ $take->getProduct->title }}">
                        </div>
                    </a>
                </div>
                ...
                <div class="w-100 min-w-0 px-2 pb-3">
                    ...
                    <div class="d-flex align-items-center gap-2 mb-2">
                        <div class="d-flex gap-1 fs-xs">
                            @php
                                $averageRating = Modules\Product\Http\Controllers\Frontend\MasterController::calculateAverageRating($take->getProduct->id);
                            @endphp
                            @for ($i = 1; $i <= 5; $i++)
                                @if ($i <= $averageRating)
                        ...
                        <span class="text-body-tertiary fs-xs">({{ \App\Models\Productcoms::where('status',1)->where('product_token',$take->getProduct->product_token)->count() }})</span>
                    </div>
                ...
                {{ number_format($price, 2, ',', '.') }} ₺
                        ...
                        {{-- Sepete ekle --}}
                        <a href="{{ route('product_detail', $take->getProduct->slug . '-' . $take->getProduct->product_token) }}"
