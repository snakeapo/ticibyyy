...
@section('content')
    @php
        ...
        $title = optional($product)->title ?: optional($item)->custom_title ?: 'Mezat ürünü';
    ...
    <div class="container py-5">
        <div class="mb-4">
            <h3 class="mb-1">Mezat Siparişini Tamamla</h3>
            <p class="text-muted mb-0">{{ optional($order->auction)->title }} mezatından kazandığınız ürün için adres, kargo ve ödeme bilgilerini tamamlayın.</p>
        </div>

        <form action="{{ route('auction_live_checkout_post', $order) }}" method="POST" id="auction-checkout-form">
            @csrf
                    ...
                    <div class="accordion d-flex flex-column gap-5 pe-lg-4 pe-xl-0" id="checkout">
                        <div class="accordion-item d-flex align-items-start border-0">
                            ...
                            <div class="w-100 ps-3 ps-md-4">
                                <div class="d-flex align-items-center justify-content-between gap-3">
                                    <h2 class="accordion-header h5 mb-0 me-3" id="deliveryInfoHeading">Kayıtlı adreslerim</h2>
                                    @if($hasAddresses)
                                        <button type="button" class="btn btn-sm btn-outline-primary" id="new-address-toggle">Yeni adres gir</button>
                                    @endif
                                </div>
                                <div class="pt-3 pt-md-4">
                                    <input type="hidden" name="user_address" id="selected-address-id" value="{{ old('user_address', $selectedAddressId) }}">
                                    @forelse($address as $take)
                                        <div class="checkout-address-card border rounded-4 p-3 mb-3 {{ $selectedAddressId == $take->id ? 'border-primary' : '' }}" data-address-card>
                                            <div class="nav flex-nowrap align-items-start justify-content-between gap-3">
                                                ...
                                                <button type="button" class="btn btn-sm {{ $selectedAddressId == $take->id ? 'btn-primary' : 'btn-outline-primary' }} flex-shrink-0" data-address-select data-address-id="{{ $take->id }}">
                                                    {{ $selectedAddressId == $take->id ? 'Seçildi' : 'Seç' }}
                                                </button>
                                            </div>
                                        </div>
                                    @empty
                                        <div class="alert alert-info mb-0">Kayıtlı adresiniz bulunmuyor. Siparişi tamamlamak için aşağıdaki adres alanını doldurun.</div>
                                    @endforelse
                                </div>
                            </div>
                        </div>
                        ...
                        <div class="d-flex align-items-start {{ $showNewAddress ? '' : 'd-none' }}" id="new-address-section">
                            ...
                            <div class="w-100 ps-3 ps-md-4">
                                ...
                                <div class="row">
                                    <div class="col-lg-6 mb-2"><label class="form-label">Adres başlığı</label><input type="text" class="form-control" name="address_title" value="{{ old('address_title') }}" data-new-address-field {{ $showNewAddress ? 'required' : 'disabled' }}></div>
                                    <div class="col-lg-6 mb-2"><label class="form-label">Telefon</label><input type="tel" class="form-control" name="phone" value="{{ old('phone') }}" data-new-address-field {{ $showNewAddress ? 'required' : 'disabled' }}></div>
                                    <div class="col-lg-6 mb-2"><label class="form-label">İl</label><input type="text" class="form-control" name="city" value="{{ old('city') }}" data-new-address-field {{ $showNewAddress ? 'required' : 'disabled' }}></div>
                                    <div class="col-lg-6 mb-2"><label class="form-label">İlçe</label><input type="text" class="form-control" name="town" value="{{ old('town') }}" data-new-address-field {{ $showNewAddress ? 'required' : 'disabled' }}></div>
                                    <div class="col-lg-4 mb-2"><label class="form-label">Posta kodu</label><input type="text" class="form-control" name="postal_code" value="{{ old('postal_code') }}" data-new-address-field {{ $showNewAddress ? 'required' : 'disabled' }}></div>
                                    <div class="col-lg-8 mb-2"><label class="form-label">Adres</label><input type="text" class="form-control" name="address" value="{{ old('address') }}" data-new-address-field {{ $showNewAddress ? 'required' : 'disabled' }}></div>
                                </div>
                            </div>
                        </div>

                        <div class="d-flex align-items-start">
                            <div class="d-flex align-items-center justify-content-center bg-body-secondary text-body-secondary rounded-circle fs-sm fw-semibold lh-1 flex-shrink-0" style="width: 2rem; height: 2rem; margin-top: -.125rem"><i class="ci-check fs-base"></i></div>
                            <div class="w-100 ps-3 ps-md-4">
                                <h2 class="h5 mb-0">Ödeme</h2>
                                <div class="mb-4" role="list">
                                    @if($requiresBalance)
                                        <div class="alert alert-success mt-4 mb-0">Bu bakiyeli mezatta kazanma tutarı teklif sırasında bakiyenizden tahsil edildi.</div>
                                ...
                                <button type="submit" class="btn btn-lg btn-primary w-100 d-none d-lg-flex justify-content-center" @if($cargos->isEmpty()) disabled @endif>Siparişi Onayla</button>
                            </div>
                        </div>
                    </div>
                </div>
                        ...
                        <div class="bg-body-tertiary rounded-5 p-4 mb-3">
                            <div class="p-sm-2 p-lg-0 p-xl-2">
                                <div class="border-bottom pb-4 mb-4">
                                    <h5 class="mb-4">Kazandığınız Ürün</h5>
                                    <div class="d-flex align-items-center gap-3">
                                        <img src="{{ $imagePath }}" class="d-block rounded-3 object-fit-cover" style="width:96px;height:96px" alt="{{ $title }}" onerror="this.src='/extra/img/photo.png'">
                                        <div class="w-100 min-w-0">
                                            <h6 class="fs-sm mb-1">{{ $title }}</h6>
                                            <div class="fs-xs text-body-secondary">Kazanma şekli: {{ $order->win_type }}</div>
                                            <div class="fs-sm fw-semibold">{{ number_format($order->final_price, 2) }} TL</div>
                                        </div>
                                    </div>
                                </div>
                                <ul class="list-unstyled fs-sm gap-3 mb-0">
                                    <li class="d-flex justify-content-between">Ürün tutarı:<span class="text-dark-emphasis fw-medium">{{ number_format($order->final_price, 2) }} TL</span></li>
                                    <li class="d-flex justify-content-between">Kargo:<span class="text-dark-emphasis fw-medium" id="selected-cargo-amount">0,00 TL</span></li>
                                </ul>
                                <div class="border-top pt-4 mt-4">
                                    <div class="d-flex justify-content-between mb-3"><span class="fs-sm">Toplam:</span><span class="h5 mb-0 order-total-amount" data-base-total="{{ number_format($order->final_price, 2, '.', '') }}">{{ number_format($order->final_price, 2) }} TL</span></div>
                                </div>
                            </div>
                        </div>
                        <button type="submit" form="auction-checkout-form" class="btn btn-lg btn-primary w-100 d-lg-none" @if($cargos->isEmpty()) disabled @endif>Siparişi Onayla</button>
                    </div>
                </aside>
            </div>
        </form>
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    function toggleNewAddress(show) {
        ...
        newAddressFields.forEach((field) => { field.disabled = !show; field.required = show; });
        if (show && selectedAddressInput) {
            selectedAddressInput.value = '';
            addressButtons.forEach((button) => { button.classList.remove('btn-primary'); button.classList.add('btn-outline-primary'); button.textContent = 'Seç'; });
    ...
    addressButtons.forEach((button) => {
        button.addEventListener('click', function () {
            if (selectedAddressInput) selectedAddressInput.value = this.dataset.addressId;
            addressButtons.forEach((item) => { item.classList.remove('btn-primary'); item.classList.add('btn-outline-primary'); item.textContent = 'Seç'; });
            addressCards.forEach((card) => card.classList.remove('border-primary'));
            this.classList.remove('btn-outline-primary'); this.classList.add('btn-primary'); this.textContent = 'Seçildi';
            this.closest('[data-address-card]')?.classList.add('border-primary');
            toggleNewAddress(false);
        });
    });

    if (newAddressToggle) newAddressToggle.addEventListener('click', () => toggleNewAddress(true));
    ...
    cargoRadios.forEach((radio) => {
        radio.addEventListener('change', function () { updateTotal(this); });
        if (radio.checked) updateTotal(radio);
    });
});
</script>
@endsection
